...
export default function ReportDetail() {
  ...
  return <View style={styles.page}>
    ...
    {loading && !report ? <ActivityIndicator style={{ flex: 1 }} color={colors.primary} /> : report ? <ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={load} />} contentContainerStyle={styles.content}>
      ...
      {report.citizen_ai?.triage && <><Text style={styles.section}>قرار وكيل أمان AI</Text><Card>
        <View style={styles.aiHeader}><Ionicons name={report.citizen_ai.triage.is_incident_report ? 'shield-checkmark' : 'remove-circle'} size={25} color={report.citizen_ai.triage.is_incident_report ? colors.primary : colors.muted} /><View style={{ flex: 1 }}><Text style={styles.aiTitle}>{relevanceLabel(report.citizen_ai.triage.incident_relevance)}</Text><Text style={styles.aiText}>{report.citizen_ai.triage.reason}</Text></View></View>
        {!!report.citizen_ai.classification && <InfoRow label="النوع المستنتج" value={incidentLabel(report.citizen_ai.classification.confirmed_incident_type)} />}
        {!!report.citizen_ai.severity && <><InfoRow label="تقدير الخطر" value={`${report.citizen_ai.severity.risk_score}% — ${priorityLabel(report.citizen_ai.severity.priority)}`} /><Text style={styles.aiReason}>{report.citizen_ai.severity.reason}</Text>{report.citizen_ai.severity.evidence?.map((item) => <Text key={item} style={styles.evidence}>• {item}</Text>)}</>}
      </Card></>}

      {!!report.media_paths?.length && <><Text style={styles.section}>المرفقات</Text>{!!report.media_blur?.length && <View style={styles.privacyStatus}><Ionicons name="shield-checkmark" size={19} color={colors.success} /><Text style={styles.privacyStatusText}>{privacySummary(report.media_blur)}</Text></View>}<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 10 }}>{report.media_paths.map((uri) => <Image key={uri} source={{ uri }} style={styles.image} />)}</ScrollView></>}
      {!!report.media_exif?.length && <><Text style={styles.section}>بيانات الصور</Text>{report.media_exif.map((metadata, index) => <Card key={`${metadata.filename}-${index}`} style={styles.metadataCard}>
        <Text style={styles.metadataTitle}>الصورة {index + 1}</Text>
        <InfoRow label="الجهاز" value={[metadata.device_make, metadata.device_model].filter(Boolean).join(' ') || 'غير متاح'} />
        <InfoRow label="وقت الالتقاط" value={metadata.captured_at ? new Date(metadata.captured_at).toLocaleString('ar-OM') : 'غير متاح'} />
        <InfoRow label="حالة العمر" value={captureLabel(metadata.captured_at_status)} />
        <InfoRow label="مطابقة الموقع" value={locationMatchLabel(metadata.location_match, metadata.distance_from_report_meters)} />
        <InfoRow label="الأبعاد" value={metadata.width && metadata.height ? `${metadata.width} × ${metadata.height}` : 'غير متاح'} />
      </Card>)}</>}
...
function InfoRow({ label, value }: { label: string; value: string }) { return <View style={styles.infoRow}><Text style={styles.infoValue}>{value}</Text><Text style={styles.infoLabel}>{label}</Text></View>; }
const relevanceLabel = (value: string) => value === 'non_incident' ? 'المحتوى لا يبدو بلاغاً حقيقياً' : value === 'credible_incident' ? 'بلاغ حادث محتمل بوضوح' : value === 'possible_incident' ? 'قد يكون بلاغاً ويحتاج تحققاً' : 'الأدلة غير كافية';
const captureLabel = (value?: string) => value === 'recent' ? 'حديثة' : value === 'old' ? 'قديمة — تحتاج مراجعة' : value === 'future' ? 'وقت غير منطقي' : 'غير معروف';
const locationMatchLabel = (value?: string, distance?: number | null) => value === 'match' ? `متطابق${distance != null ? ` (${distance}م)` : ''}` : value === 'mismatch' ? `مختلف${distance != null ? ` (${distance}م)` : ''}` : 'لا يوجد GPS للمقارنة';
const privacySummary = (items: NonNullable<Report['media_blur']>) => {
  const faces = items.reduce((total, item) => total + (item.faces || 0), 0);
  const fallback = items.some((item) => item.method === 'full_image_fallback');
  return fallback ? 'تمت حماية الخصوصية بتمويه الصورة احتياطياً' : faces > 0 ? `تم تمويه ${faces} ${faces === 1 ? 'وجه' : 'وجوه'} قبل الحفظ` : 'تم فحص الصورة وحذف بياناتها المخفية قبل الحفظ';
};
...
const styles = StyleSheet.create({
  ...
  privacyStatus: { flexDirection: 'row-reverse', alignItems: 'center', gap: 8, backgroundColor: colors.successSoft, borderRadius: 14, padding: 11, marginBottom: 10 }, privacyStatusText: { flex: 1, color: colors.success, fontSize: 11, fontWeight: '800', textAlign: 'right' },
  aiHeader: { flexDirection: 'row-reverse', alignItems: 'flex-start', gap: 10, marginBottom: 12 }, aiTitle: { color: colors.ink, fontWeight: '900', textAlign: 'right' }, aiText: { color: colors.muted, fontSize: 11, lineHeight: 18, textAlign: 'right', marginTop: 4 }, aiReason: { color: '#42526A', fontSize: 12, lineHeight: 19, textAlign: 'right', marginTop: 10 }, evidence: { color: colors.muted, fontSize: 11, lineHeight: 18, textAlign: 'right' }, infoRow: { flexDirection: 'row', justifyContent: 'space-between', gap: 12, borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 10, marginTop: 10 }, infoLabel: { color: colors.muted, fontSize: 11, textAlign: 'right' }, infoValue: { flex: 1, color: colors.ink, fontSize: 12, fontWeight: '700', textAlign: 'left' }, metadataCard: { marginBottom: 10 }, metadataTitle: { color: colors.ink, fontWeight: '900', textAlign: 'right' },
